...
export default function Home() {
  ...
  useEffect(() => {
    const fecthdata = async () => {
      ...
      console.log(events)
      console.log(events[0].args.CampaignAddress)
      // setCampaddress(events[0].args.newCampaign)
  ...
  return (
    <>
      <div className='min-h-[90vh] flex items-center justify-evenly flex-wrap gap-[20px]  bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6' style={{ marginTop: "80px" }}>
        ...
        {Loder && campaing ? <div className='w-full h-full flex justify-center items-center '><div className='lodermain'></div></div> :campaing.map((data,index)=>(
          ...
          <div className="money  flex justify-between" style={{ padding: "10px" }}>
            <span className='text-xl text-white' >Need Amount</span> <span className='text-xl text-white'>{ethers.formatEther(data.args.requireAmount)} ETH</span>
          </div>
          ...
          <div className="btn flex justify-center">
           <Link to={`/${data.args.CampaignAddress}`}> <button style={{ padding: "10px", width: "300px", marginTop: "5px" }} className="bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition-colors cursor-pointer">
              Donate
            </button></Link>
          </div>
        </div>
